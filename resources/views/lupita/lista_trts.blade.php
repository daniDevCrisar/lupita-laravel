...
@section('title', 'Lista de Transportistas')
                ...
                <table class="table table-bordered table-hover table-sm table-dark">
                    ...
                    @foreach($trts as $row)
                        <tr class="{{ $loop->odd ? 'table-secondary' : '' }} ">
                            ...
                            <td><span class="badge bg-{{ $llamadas::color_porcentaje(100-$problematicos) }}">{{  $problematicos }}   @if(!request('reporte'))  %@endif</span></td>
