...
DROP PROCEDURE IF EXISTS `sp_actualizar_todas_rutas`$$

CREATE PROCEDURE `sp_actualizar_todas_rutas`()
BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE v_ref VARCHAR(50);
    DECLARE v_continue BOOLEAN DEFAULT TRUE;
    DECLARE cur CURSOR FOR
        SELECT ref FROM referencias
        WHERE ruta_id IS NULL AND titulo_viaje IS NOT NULL;
...
DELIMITER ;
CALL sp_actualizar_todas_rutas();
